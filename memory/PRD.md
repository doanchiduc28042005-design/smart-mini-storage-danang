...
- ✅ **Real map integration** (Leaflet + OpenStreetMap) in all 3 apps
- ✅ **Location Picker** with Da Nang district presets + boundary check
- ✅ **4-tabs Box Management** by status
- ✅ **Customer Auth (JWT)** - register/login (email or phone) + protected dashboard
- ✅ **Terms & Conditions page** with legal content
- ✅ **Mandatory terms checkbox** on registration
- ✅ **Customer Self-Create-Order** - customer creates pickup requests from dashboard with item description, pickup time, mandatory no-prohibited-items commitment. Auto-generates BOX + QR. Admin sees 'KH tự tạo' badge in boxes management.
